...
include_once '../admin/paypal_config.php';  
...
if (strcmp($res, "VERIFIED") == 0 || strcasecmp($res, "VERIFIED") == 0) { 
    ...
    if(!empty($txn_id)){ 
        ...
        $prevPayment = $Conectar::conexion->query("SELECT id FROM user_subscriptions WHERE txn_id = '".$txn_id."'"); 
         
        if($prevPayment->num_rows > 0){ 
            exit(); 
        }else{ 
            // Insert transaction data into the database 
            $insert = $Conectar::conexion->query("INSERT INTO user_subscriptions(user_id,validity,valid_from,valid_to,item_number,txn_id,payment_gross,currency_code,subscr_id,payment_status,payer_email) VALUES('".$custom."','".$subscr_period."','".$subscr_date."','".$subscr_date_valid_to."','".$item_number."','".$txn_id."','".$payment_gross."','".$currency_code."','".$subscr_id."','".$payment_status."','".$payer_email."')"); 
             
            // Update subscription id in the users table 
            if($insert && !empty($custom)){ 
                $subscription_id = $Conectar::conexion->insert_id; 
                $update = $db->query("UPDATE user SET subscription_id = {$subscription_id} WHERE id = {$custom}"); 
            } 
        } 
    } 
} 
